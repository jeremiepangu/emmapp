...
import { api, ClientSituation } from '../api';
...
/**
 * Situation consolidée d'un client : ce qu'il doit en argent et en contenants,
 * et ce dont il dispose à son crédit. Partagée par tous les écrans de vente.
 */
export default function ClientSituationPanel({ clientId, compact, refreshKey, onLoaded }: Props) {
  ...
  const load = useCallback(() => {
    ...
    // onLoaded est volontairement hors dépendances : les appelants passent
    // souvent une lambda recréée à chaque rendu.
    ...
    <div className={`client-situation${compact ? ' is-compact' : ''}`}>
      <div className="client-situation-grid">
        ...
        <div className={`client-situation-item${cash.advance > 0 ? ' is-credit' : ''}`}>
          ...
          {cash.advance > 0 && <small>imputée à la prochaine commande</small>}
        </div>
